...
def _json_from_streamlit_secret(key: str):
    ...
    # Fallback: credentials_json / token_json en raíz
    if value is None:
        try:
...
def build_bcp_query(days: int = 30, medio=None) -> str:
    """Query para notificaciones BCP usando rango de días dinámico."""
    return (
...
def fetch_bcp_consumption_emails(
    ...
    print(f"Query Gmail: {query} -> {len(msgs)} correos")
